...
input.file="~/10X-3_summary.txt" # <- useful if you want to specify a summary to run manually
...
<!--|*Property*|*Statistic*|
|:---------|:----------|
|Total giga-bases|`r I(total.bases)`|
|Total reads (millions)|`r I(total.reads)`|
|Reads >= Q7, fraction of total|`r I(total.pass.reads)` (`r I((total.pass.reads/total.reads)*100)`%)|
|Reads >= Q10, fraction of total|`r I(total.ten.reads)` (`r I((total.ten.reads/total.reads)*100)`%)|
|Longest read|`r I(max.length)`|
|Longest passing read|`r I(max.pass.length)`|
|N50 read|`r I(N50.read)`|
|L50 read length|`r I(N50.length)`|
|Mean length|`r I(mean.length)`|
|Median length|`r I(median.length)`|
|Mean quality|`r I(mean.q)`|
|Median quality|`r I(median.q)`|
|Active sensors|`r I(active.channels)`|
|Runtime (hours)|`r I(run.length)`| !-->
