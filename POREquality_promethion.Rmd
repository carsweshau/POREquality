---
title: "BC12" #"A POREquality report"
output: 
...
opts_chunk$set(fig.width=12, fig.height=8, fig.path='images/', include=FALSE, echo=FALSE, warning=FALSE, message=FALSE)
```
...
input.file="~/BC12_seq_sum.txt" # <- useful if you want to specify a summary to run manually
...
#seq_dt <- read.delim.ffdf(file=input.file, header=T)
#seq_dt <- sample.df("~/NXBAA128793_summary.txt",n=1000,p=0.2) #n is number of lines to read at once, p is fraction to sample
...
seq_dt <- subset(seq_dt, seq_dt$mean_qscore_template > 0) # removes negative quality score reads (?!)
```
...
```{r channel_activity}
seq_dt <- seq_dt[sample(.N,400000)] # Subsampling for PromethION runs
...
ggplot(bnum, aes(x=sequence_length_template)) +
    stat_density(aes(y=..count..), colour=hm.palette(9)[8], fill=hm.palette(9)[4], alpha=0.3) +
    scale_x_continuous(breaks=seq(from=0,to=mean.length*4,by=250), expand=c(0,0), limits=c(0,mean.length*4)) +
...
#     scale_y_continuous(expand=c(0,0)) +
#     #scale_x_log_eng() + #mean.length*5), breaks=pretty(bnum$sequence_length_template,n=40)) + # pretty(bnum$sequence_length_template,n=40) need to fix this to actually be dynamic and not empty for PromethION runs
#     xlab("Read length\n") +
#     ylab("Number of reads\n") +
